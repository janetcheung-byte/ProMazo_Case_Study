...
# Define the sales target (set by user)
SALES_TARGET = 20000000  # Example: $20M in new sales

# Create the LP problem for hitting the sales target while maximizing margin
model_sales_margin = pulp.LpProblem("HitSalesTarget_MaximizeMargin", pulp.LpMaximize)
...
model_sales_margin += pulp.lpSum(
    (row["Margin (%)"] / 100.0) * new_sales_vars[idx] for idx, row in df.iterrows()
), "Total_Margin"

# Constraint: The sum of all new sales must be at least the user-defined sales target
model_sales_margin += pulp.lpSum(new_sales_vars.values()) >= SALES_TARGET, "Sales_Target_Constraint"

# Constraints: BrandTotal = sum of NewSales for all segments of that Brand
for brand in df["Brand"].unique():
...
# Solve the Model
solution_status_sales_margin = model_sales_margin.solve(pulp.PULP_CBC_CMD(msg=False))
print("Solution Status:", pulp.LpStatus[solution_status_sales_margin])

if pulp.LpStatus[solution_status_sales_margin] == "Optimal":
    total_margin = pulp.value(model_sales_margin.objective)
    print(f"\nMaximum Achievable Total Margin while Meeting Sales Target: ${total_margin:,.2f}\n")
    ...
    df.to_csv("optimized_sales_target_results.csv", index=False)
    print("Optimized results saved to 'optimized_sales_target_results.csv'")
